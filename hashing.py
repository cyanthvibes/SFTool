...
def get_pathname_and_hashes(): #
    ...
    filesize = int(input("Wat is de grootte (MB's) van de bestanden die u wilt hashen? "))  # Vraagt aan de gebruiker wat de bestandsgrootte moet zijn
    filesize = filesize * 1048576  # Conversie van 1 byte naar 1MB
...
def convert_md5_to_sha1():
    ...
    lines = [line.rstrip('\n') for line in open('system_hashes.txt')]  # Virusshare matches worden omgezet in list
    ...
    for key, value in path_dict.items(): # De loop gaat over alle items in de dictionary
        ...
        if hash in lines: # Als de hash voorkomt in de dictionary, doe het volgende:
            ...
            with open('malware_sha_path.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerow([checksum, padnaam])    # De SHA! waarde en de padnaam van de malware worden weggeschreven naar het bestand
